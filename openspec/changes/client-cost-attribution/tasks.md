...
- [ ] 6.4 Adapter Gemini — `sem fonte utilizável`: CLI real (`agy`/Antigravity) com autenticação funcional, headless JSON confirmado com chamada real. Oito fontes locais investigadas sem achar uso/custo (ver `src/adapters/mod.rs::cobertura_v0_0`). Confirmado pelo repositório oficial (issues públicas): uso é rastreado só no servidor via endpoint interno `v1internal:retrieveUserQuota` (issue #387), sem exportação local (issue #366) e sem subcomando de CLI (issue #543) — ambas ainda feature request. Lacuna de produto reconhecida pela comunidade, não falha de investigação
