...
**ZCode não vira task de adapter.** Investigado e resolvido como `sem fonte
utilizável`: só existe app nativo Mac (Electron), sem CLI, sem log de uso
documentado — armazenamento interno é LevelDB/protobuf sem schema estável
(engenharia reversa de formato não documentado é categoria de risco maior
que qualquer coisa desta change). Não faz sentido como task com checkbox
pendente para sempre; a investigação e o motivo ficam registrados em
`src/adapters/mod.rs::cobertura_v0_0()`.
...
- [x] 8.2 Verificar as invariantes de integridade contra um ledger populado, incluindo registros órfãos e de custo desconhecido (`integridade_de_ledger_populado_realista`: 5 estados legítimos coexistindo — atribuído, órfão, custo desconhecido, supersessão de custo tardio — só a linha deliberadamente corrompida viola)
- [x] 8.3 Medir as consultas de custo com volume sintético equivalente a doze meses e confirmar o limite de 200ms que D-1 estabelece como critério de revisão. 200 mil registros sintéticos (premissa documentada no teste, blueprint não fixa número), 20 clientes, 5 providers. Casos de uso real ficam bem dentro do limite: `consumo_do_cliente` 15,7ms, `consumo_no_periodo` com recorte de 1 mês 23,2ms, `nao_atribuidos` 43µs. **Achado real**: `consumo_no_periodo` sem nenhum recorte (histórico de 12 meses inteiro, caso de `--by` sem `--period`) passa de 200ms — 247ms. Causa identificada por benchmark isolado, não suposição: não é a consulta SQL (`COUNT(*)` 2ms, seleção de 4 colunas 40ms para as mesmas 200 mil linhas), é materializar o `UsageRecord` completo por linha em Rust. Índice `idx_usage_time` adicionado mesmo assim (ajudava a busca, não o gargalo real). Documentado como limite conhecido no próprio teste — correção real seria agregação do lado do SQL, trabalho novo de storage desproporcional a esta task; D-1 existe exatamente para revisitar isso se o dado real confirmar que importa
- [x] 8.4 Confirmar isolamento entre clientes por teste que falha se uma consulta escopada retornar registro de outro cliente ou não atribuído. Dois modos de falha distintos, dois testes: `isolamento_entre_clientes` (não vaza para outro cliente) e `consulta_de_cliente_nao_inclui_nao_atribuidos` (não vaza órfão) — o segundo era uma lacuna real, fechada nesta task
- [x] 8.5 Confirmar que `brian costs --unattributed` retorna vazio em um cenário de ledger íntegro — teste de integridade do D-16 (`ledger_integro_nao_tem_consumo_sem_dono`, já existente do grupo 4)
- [x] 8.6 Confirmar que consumo por assinatura produz custo equivalente e custo pago inexistente, e que nenhuma consulta apresenta o equivalente como valor pago. Coberto em 3 camadas (domain, custo, comandos) mais **1 bug real encontrado nesta task**: `formatar_unattributed` colapsava pago/equivalente numa coluna "custo" genérica via `.or()` — se pago fosse `None`, o equivalente aparecia ali sem rótulo, indistinguível de valor pago de verdade. Corrigido para colunas separadas `custo_pago`/`custo_equivalente`, como em toda outra visão
- [x] 8.7 Confirmar que `./scripts/verificar-invariantes.sh` passa e que ele falha quando uma violação de D-9 ou de soma de custos é plantada deliberadamente. Formalizado em `scripts/testar-verificar-invariantes.sh` — planta e limpa as duas violações, falha o próprio meta-teste se o guard não pegar nenhuma delas. Ligado ao CI (job `invariantes`), roda em todo push. Corrigido de passagem: CI usava `cargo clippy` sem `--all-targets` (não checava código de teste) — alinhado com o comando local e os protocolos
